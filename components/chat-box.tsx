...
const getServerSnapshot = () => 'invitado';
    ...
    <div className="flex h-screen w-full bg-[#050505] text-zinc-100 overflow-hidden font-sans">
      <div className="flex-1 flex flex-col min-w-0 relative bg-black">
          ...
          <div className="max-w-3xl mx-auto w-full">
            {messages.length === 0 && !isLoading && (
              <div className="h-[50vh] flex flex-col items-center justify-center text-center animate-in fade-in zoom-in duration-700">
                <div className="w-16 h-16 rounded-3xl bg-zinc-900/50 border border-zinc-800 flex items-center justify-center text-3xl mb-6 shadow-2xl font-sans">
                  🍿
                </div>
                <h2 className="text-2xl font-bold text-white tracking-tight">Tu cartelera personal</h2>
                <p className="text-zinc-500 text-sm mt-2 max-w-xs italic text-center">
            ...
            {error && (
              <div className="mx-auto max-w-md p-4 bg-red-500/5 border border-red-500/20 rounded-2xl text-red-500 text-[11px] text-center mt-8 font-mono uppercase">
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 bg-gradient-to-t from-black via-black/95 to-transparent z-20">
          ...
          >
            <input
              className="w-full bg-zinc-900/40 border border-zinc-800/50 rounded-2xl pl-6 pr-14 py-4 text-sm text-zinc-100 placeholder:text-zinc-700 focus:outline-none focus:ring-1 focus:ring-blue-500/30 transition-all backdrop-blur-md shadow-2xl"
              value={input}
      ...
      {showDebug && (
        <aside className="w-[450px] border-l border-zinc-900 bg-[#070707] z-40 animate-in slide-in-from-right duration-300 shadow-2xl overflow-y-auto">
